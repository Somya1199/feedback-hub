...
# # # def get_mappings():
# #     # try:
# #     #     print("Fetching mapping sheet data...")
# #     #     sheet_id = os.getenv('GOOGLE_SHEET_ID_MAPPING')
# #     #     sheet_name = os.getenv('MAPPING_SHEET_NAME')
# #     #     print(f"Sheet ID: {sheet_id}")
# #     #     print(f"Sheet Name: {sheet_name}")
        
# #     #     data = read_google_sheet(sheet_id, sheet_name)
# #     #     print(f"Raw data from sheet: {data}")
# #     #     print(f"Number of rows: {len(data)}")
        
# #     #     if not data or len(data) < 2:
# #     #         print("No data or only headers found")
# #     #         return jsonify({"success": False, "error": "No data found in mapping sheet", "data": []})
            
# #     #     # Convert to EmployeeMapping objects
# #     #     headers = data[0]
# #     #     print(f"Headers: {headers}")
        
# #     #     mappings = []
# #     #     for i, row in enumerate(data[1:], 1):
# #     #         if len(row) >= 2:  # At least Email and Ldap
# #     #             mapping = {
# #     #                 "Email": row[headers.index("Email")] if "Email" in headers else "",
# #     #                 "Ldap": row[headers.index("Ldap")] if "Ldap" in headers else "",
# #     #                 "Process": row[headers.index("Process")] if "Process" in headers else "",
# #     #                 "POC": row[headers.index("POC")] if "POC" in headers else "",
# #     #                 "Manager": row[headers.index("Manager")] if "Manager" in headers else "",
# #     #                 "AccountManager": row[headers.index("AccountManager")] if "AccountManager" in headers else ""
# #     #             }
# #     #             print(f"Row {i}: {mapping}")
# #     #             mappings.append(mapping)
        
# #     #     print(f"Total mappings parsed: {len(mappings)}")
# #     #     return jsonify({"success": True, "data": mappings})
        
# #     # except Exception as e:
# #     #     print(f"Error in get_mappings: {str(e)}")
# #     #     import traceback
# #     #     traceback.print_exc()
...
# #         # List all available sheets in this spreadsheet
# #         try:
# #             service = get_google_sheets_service()
# #             spreadsheet = service.spreadsheets().get(spreadsheetId=sheet_id).execute()
# #             sheets = spreadsheet.get('sheets', [])
# #             print(f"Available sheets in this spreadsheet:")
# #             for sheet in sheets:
...
# #         if not data or len(data) < 2:
# #             print("ERROR: No data or only headers found!")
# #             return jsonify({"success": False, "error": "No data found in mapping sheet", "data": []})
...
# #         mappings = []
# #         for i, row in enumerate(data[1:], 1):
# #             if row:  # Check if row has any data
# #                 mapping = {}
# #                 for j, header in enumerate(headers):
# #                     if j < len(row):
# #                         mapping[header] = row[j]
# #                     else:
# #                         mapping[header] = ""
                
# #                 # Check if this row has at least Email or Ldap
# #                 if mapping.get('Email') or mapping.get('Ldap'):
# #                     print(f"Row {i} - Valid mapping: {mapping}")
# #                     mappings.append(mapping)
# #                 else:
# #                     print(f"Row {i} - Skipped (no Email or Ldap): {mapping}")
        
# #         print(f"\nTotal valid mappings parsed: {len(mappings)}")
# #         print("=" * 50)
...
# @app.route('/api/mappings')
# def get_mappings():
#     """Alternative endpoint for employee mappings (for admin dashboard)."""
#     try:
#         print("=" * 50)
#         print("FETCHING MAPPING SHEET DATA")
#         print("=" * 50)
...
#         if not sheet_id:
#             print("ERROR: GOOGLE_SHEET_ID_MAPPING not set in .env")
#             return jsonify({"success": False, "error": "MAPPING_SHEET_ID not configured", "data": []})
        
#         client = get_google_sheets_client()
#         if not client:
#             print("ERROR: Google Sheets client not initialized")
#             return jsonify({"success": False, "error": "Failed to connect to Google Sheets", "data": []})
...
#         except Exception as e:
#             print(f"❌ Failed to open spreadsheet: {e}")
#             return jsonify({"success": False, "error": f"Failed to open spreadsheet: {str(e)}", "data": []})
...
#             return jsonify({
#                 "success": False, 
#                 "error": f"Worksheet '{sheet_name}' not found. Available: {worksheet_names}",
#                 "data": [],
#                 "available_sheets": worksheet_names
#             })
...
#     print("   GET  /api/health          - Health check")
#     print("   GET  /api/questions       - Get survey questions")
#     print("   GET  /api/mapping         - Get management mapping (filter by email)")
#     print("   GET  /api/mappings        - Get all employee mappings (for admin)")
#     print("   GET  /api/responses       - Get all feedback responses")
#     print("   POST /api/submit-feedback - Submit new feedback")
#     print("   GET  /api/detect-email    - Detect email from SSO headers")
#     print("   GET  /api/decode-token    - Decode email from token")
#     print("=" * 60)
...
GOOGLE_SHEET_ID_MAPPING = os.getenv('GOOGLE_SHEET_ID_MAPPING')
GOOGLE_SHEET_ID_QUESTIONS = os.getenv('GOOGLE_SHEET_ID_QUESTIONS')
GOOGLE_SHEET_ID_RESPONSES = os.getenv('GOOGLE_SHEET_ID_RESPONSES')

MAPPING_SHEET_NAME = os.getenv('MAPPING_SHEET_NAME', 'Sheet1')
QUESTIONS_SHEET_NAME = os.getenv('QUESTIONS_SHEET_NAME', 'Sheet1')
RESPONSES_SHEET_NAME = os.getenv('RESPONSES_SHEET_NAME', 'Sheet1')
...
def get_questions():
    try:
        print(f"Fetching questions from sheet: {GOOGLE_SHEET_ID_QUESTIONS}")
        
        # Get all data from questions sheet
        range_name = f"{QUESTIONS_SHEET_NAME}!A:Z"
        data = get_sheet_data(GOOGLE_SHEET_ID_QUESTIONS, range_name)
...
# Get management mapping
@app.route('/api/mapping', methods=['GET'])
def get_mapping():
    try:
        user_email = request.args.get('email')
        print(f"Fetching mapping for user: {user_email or 'all users'}")
        
        # Get all data from mapping sheet
        range_name = f"{MAPPING_SHEET_NAME}!A:Z"
        data = get_sheet_data(GOOGLE_SHEET_ID_MAPPING, range_name)
        ...
        if user_email:
            ...
            for item in mapping_data:
                ...
                item_ldap = (item.get('Ldap') or '').lower()
                
                # Check if user email matches Email or Ldap field
                if item_email == user_email_lower or item_ldap == user_email_lower:
                    filtered_data.append(item)
            
            mapping_data = filtered_data
        ...
        return jsonify({
            ...
            'count': len(mapping_data),
            'headers': headers,
            'filtered_by': user_email or 'all'
        })
        
    except Exception as e:
        print(f"Error in /api/mapping: {e}")
        return jsonify({
            'success': False,
            'error': str(e),
            'data': []
...
def submit_feedback():
    try:
        ...
        print(f"Received feedback submission with {len(feedback_data)} fields")
        ...
        # Get existing data to determine column order
        range_name = f"{RESPONSES_SHEET_NAME}!A1:Z1"  # Get headers only
        existing_headers = get_sheet_data(GOOGLE_SHEET_ID_RESPONSES, range_name)
        
        # Prepare row data
        if existing_headers and len(existing_headers) > 0:
            # Sheet has headers, match column order
            headers = existing_headers[0]
            row_data = []
            for header in headers:
                row_data.append(feedback_data.get(header, ''))
        else:
            # No headers yet, create them from feedback data keys
            headers = list(feedback_data.keys())
            row_data = list(feedback_data.values())
            
            # First, write headers
            header_result = append_to_sheet(
                GOOGLE_SHEET_ID_RESPONSES,
                f"{RESPONSES_SHEET_NAME}!A1",
                [headers]
            )
            print(f"Headers written: {header_result}")
        
        # Append the data row
        append_range = f"{RESPONSES_SHEET_NAME}!A:Z"
        ...
        print(f"Feedback appended successfully: {append_result}")
        ...
        print(f"Error in /api/submit-feedback: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

# Get feedback responses (for admin)
@app.route('/api/responses', methods=['GET'])
def get_responses():
    try:
        print("Fetching feedback responses")
        
        range_name = f"{RESPONSES_SHEET_NAME}!A:Z"
        data = get_sheet_data(GOOGLE_SHEET_ID_RESPONSES, range_name)
...
def test_endpoint():
    return jsonify({
        ...
        'timestamp': datetime.now().isoformat(),
        'sheets_configured': {
            'mapping': bool(GOOGLE_SHEET_ID_MAPPING),
            'questions': bool(GOOGLE_SHEET_ID_QUESTIONS),
            'responses': bool(GOOGLE_SHEET_ID_RESPONSES)
        }
    })

# Get current user (placeholder - you can implement proper auth later)
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("Starting Feedback System Backend")
    print("=" * 50)
    print(f"MAPPING SHEET ID: {GOOGLE_SHEET_ID_MAPPING}")
    print(f"QUESTIONS SHEET ID: {GOOGLE_SHEET_ID_QUESTIONS}")
    print(f"RESPONSES SHEET ID: {GOOGLE_SHEET_ID_RESPONSES}")
    print("=" * 50)
    
    # Check if credentials file exists
    if not os.path.exists(SERVICE_ACCOUNT_FILE):
        print(f"❌ ERROR: {SERVICE_ACCOUNT_FILE} not found!")
        print("Please download your service account credentials JSON file from Google Cloud Console")
        print("and save it as 'credentials.json' in the same directory as this file.")
        exit(1)
